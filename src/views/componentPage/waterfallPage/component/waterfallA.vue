...
export default{
	data(){return{
		waterfall_box_width: 0,/* 容器宽高 */
		waterfall_box_height: 0,
		waterfall_col_num: 0, // 列数
		waterfall_col_height_list:[], // 每列最大高度
		waterfall_list: [], // 瀑布流用到的数据
	}},
	props:{
		img_list: [],
		img_width: number,// 图片宽度
		img_margin_bottom: number,// 图片下边距
		img_margin_right: number,// 图片右边距
	},
	computed:{
		waterfall_box_height_exect(){// 容器的高度 拼上单位
	...
	methods:{
		...
		afreshLayoutWaterFall() {
		  const waterfall_box_dom = this.$refs.waterfall_box;
